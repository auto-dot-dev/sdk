...
async function apiGet(path: string, apiKey: string): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`API error ${response.status}: ${(body as { error?: string }).error ?? response.statusText}`)
  }
...
export function buildApiCommands(): Command[] {
  ...
  const listings = outputOptions(
    ...
    .action(async (options) => {
      ...
      if (options.priceMax) params.set('price_max', options.priceMax)
      if (options.milesMax) params.set('miles_max', options.milesMax)
  ...
  const payments = outputOptions(
    ...
    .option('--down <amount>', 'Down payment amount')
